<template>
  <el-card>
    ...
    <!-- 入库对话框 -->
    <el-dialog v-model="dialogVisible" title="入库" width="400px" :close-on-click-modal="false">
    ...
    <el-dialog v-model="deleteDialogVisible" title="确认删除" width="300px">
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="deleteLoading" @click="handleDeleteRecord">确认</el-button>
      </template>
    </el-dialog>
  </el-card>
</template>
...
const handleDeleteRecord = async (): Promise<void> => {
  ...
  try {
    ...
    // 删除入库记录
    const result = (await apiService.deleteInRecord(numericRecordId)) as {
      success: boolean
      error?: string
    }
    if (result.success) {
      // 更新耗材数量（入库记录删除后，库存数量应该减少）
      // 明确指定类型
      ...
      if (consumable) {
        ...
        console.log(
          `已将耗材 ${consumable.itemid} 的库存从 ${consumable.quantity} 减少到 ${newQuantity}`
        )
        await apiService.insertLog('删除', '耗材入库记录', `删除耗材入库记录，耗材ID: ${consumable.itemid}`, sessionStorage.getItem('username'));
      }
...
const fetchConsumables = async (): Promise<void> => {
  console.log('开始调用 getConsumables 方法')
  try {
    // 假设这里传入空字符串作为查询参数
...
const handleAddRecord = async (): Promise<void> => {
  ...
  try {
    ...
    console.log('获取到的耗材信息:', consumable)
    ...
    // 先添加入库记录
    const addRecordResult = await apiService.addRecord(recordData)
    console.log("入库记录是",recordData)
    // 检查 addRecordResult 并处理
    if (addRecordResult==0) {
      ElMessage.error('添加入库记录失败')//?????
      return
    }

    // 再更新耗材库存（确保顺序正确）
     console.log("开始更新耗材库存")
    // 将 consumable.itemid 转换为 string 类型
    ...
    console.log(
      `入库成功: 耗材 ${consumable.itemid} 的库存已从 ${consumable.quantity} 增加到 ${newQuantity}`
    )
    if (updateResult.success) {
      ElMessage.success('入库操作成功，耗材数量更新操作成功')
    }
    ...
    const errorMessage = e instanceof Error ? e.message : String(e)
    ElMessage.error('入库操作失败: ' + errorMessage)
